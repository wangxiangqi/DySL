...
def train_model(model_name: str, model, fea_constructor, prob_matrix, influ_matrix, learning_rate: float, λ, γ, ckpt_dir, idx_split_args: dict={'ntraining':200,
 'nstopping':400,  'nval':10}, stopping_args: dict=stopping_args, test: bool=False, device: str='cuda', torch_seed: int=None, print_interval: int=10, batch_size=None, timestamp=1) -> Tuple[(nn.Module, dict)]:
    ...
    for epoch in range(early_stopping.max_epochs):
        ...
        for i, influ_mat in enumerate(influ_matrix):
            try:
                attr_mat = temp_attr_mat_dict[i]
            except KeyError:
                seed_vec = influ_mat[0, :]
                #print(seed_vec.shape[0])
